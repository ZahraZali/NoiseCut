...
def noisecut(file, component, plotspec= 'no'):
    '''
    Reduce noise from all the components of the OBS data using HPS noise reduction algorithms.
    '''
...
# 1Hz=164
    ...
    S_filter = librosa.decompose.nn_filter(S_full1,aggregate=np.median,metric='cosine',width=200)
    ...
    mask_v = librosa.util.softmask(S_full1 - S_filter,margin_v * S_filter,power=power)

    S_foreground = mask_v * S_full1
    S_background = mask_i * S_full1

    D_harmonic, D_percussive = librosa.decompose.hpss(S_full2,kernel_size=80,  margin=5)
    S_background=S_background+D_harmonic
    
    
    f=S_background*phase
    L = x.shape[0]
    new = librosa.istft(f, hop_length=hop_length, win_length=win_length, window='hann', length=L)
    z=x-new
    stats = trace[0].stats
    stats.location= 'NC'
    ...
    if plotspec=='yes':
        ...
        x2=trace2[0].data
        y2=x2.astype(float)
        Noisereduced=librosa.stft(y2, n_fft= n_fft, hop_length=hop_length, win_length=win_length)

        fig= plt.figure(figsize=(18, 9))
        plt.subplot(3, 2, 1)
        librosa.display.specshow(librosa.power_to_db(np.abs(S_full)),y_axis='log', sr=sr)
        ...
        plt.subplot(3, 2, 3)
        librosa.display.specshow(librosa.power_to_db(np.abs(S_background)), sr=sr,y_axis='log')
        plt.ylabel('Frequency (Hz)', fontsize= 14)
        ...
        plt.subplot(3, 2, 5)
        librosa.display.specshow(librosa.power_to_db(np.abs(Noisereduced)), sr=sr,y_axis='log')
        plt.ylabel('Frequency (Hz)', fontsize= 14)
        ...
        plt.subplot(3, 2, 2)
        librosa.display.specshow(librosa.power_to_db(np.abs(S_full)), sr=sr,y_axis='log')
        plt.ylabel('Frequency (Hz)', fontsize= 14)
        plt.title('Full spectrogram', fontsize= 14)
        plt.ylim(0,160)
